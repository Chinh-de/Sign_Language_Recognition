...
def update_last_w_s(text, is_word):
    ...
    try:
        ...
        last_word_or_sentence.get_nowait()
    except:
        pass
    ...
    if not is_word:  # Nếu là câu
        ...
        try:
            count = 1 if last_word_or_sentence.empty() else last_word_or_sentence.queue[0][1] + 1
        except:
            count = 1

    # Cập nhật queue với (text, count)
    try:
        last_word_or_sentence.put_nowait((text, count))
    except Full:
        pass
...
class Recognizer:
    ...
    def stop(self):
        ...
        # Dừng worker
        self.stop_event.set()
        action_queue.put(None)  # Để giải phóng .get() trong model_worker nếu đang chờ
        ...
        last_word_or_sentence = ("", 0)  # Gán lại giá trị mặc định
